...
- 用户给出明确网站/域名并要求找站内文档、价格、公告、API、联系方式或下载入口时，用 `guanlan map "https://example.com" --query "pricing docs" --limit 80 --read-top 2` 先发现公开 URL，并读取少量代表页质量报告。`map` 不是全网搜索，也不是规模化爬取；只引用 `read_pack.readings` / `readings` 里 `read_evidence_v1.usable=true` 的正文，未读 URL 列表不能直接当事实引用。
- Agent 调用时应尽量多取结果再筛选：普通任务保持 80，复杂研究可设到 80-100；只有用户明确要求“小样本/快速看一下”时才降低 limit。
- 当 `--limit` 小于 30 时，把它当 smoke sample：可以先返回线索，但不要用 5-10 条结果直接写强结论。`search --trace` 会给出 `agent_limit_advice.silent_repair_commands`，除非用户明确只要 smoke，先无感补跑 `--limit 80`，再压缩输出。
- `search` 默认附带 Evidence Mixer 影子诊断：它只估算哪些证据适合作为优先阅读候选，不删除、不重排结果。需要让 Agent 明确看到优先阅读顺序时用 `--evidence-mode assist`；排障或回滚时用 `--evidence-mode off`。
...
| “在已知网站里找文档/价格/公告/API/联系方式” | `guanlan map "https://example.com" --query "pricing docs" --limit 80 --read-top 2`，必要时再 `guanlan read "URL" --quality-report` |
...
| “查看 OpenGuanlan 浏览器补证层” | `guanlan browser-assist setup-openguanlan --json` |
| “生成 OpenGuanlan 补证执行契约” | `guanlan browser-assist run "URL" --adapter openguanlan --json` |
| “可选扩展桥配对码” | `openguanlan pair-code --json` / `openguanlan pair-reset --json` |
| “打包可选 OpenGuanlan Chrome 扩展桥” | `scripts/build_openguanlan_extension.sh` |
...
科技、AI、WPS/AI Office、开发者、工程实践类问题必须额外补一轮 RSS/精品内容流。默认显式跑 `search --scope tech_dev|wps_office` 和 `feeds curated`；AI 公司、模型、Agent、开发者平台更新类任务再补官方更新流和媒体观察流。`research --preset tech|wps_office` 只用于深查/证据包模式，且 MCP/Agent 场景应保持 `--read-top 0-2`。AI/WPS/Agent/大模型命中时，相关 feed 作为近 7 天动态和选题线索层；关键事实仍要回读原始 URL。如果 Agent 只跑了 `route` 或 `search`，还需要再跑：
