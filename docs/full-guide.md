...
看到 `观澜 / Guanlan v0.5.38`，并且 `doctor` 通过基础自检，就说明基础部署成功。
...
- 热榜、网页阅读与公开搜索之间的降级链路。
- 浏览器辅助补证计划、会话契约与用户授权可见页入库，帮助 Agent 在公开读取不足时保留边界清楚的补充证据；Rednote 按小红书同类公开笔记处理，动态页采集强调内容就绪信号、滚动充分性和 `partial_reason`，不把固定等待或空结果当作成功。
- 默认只读、低扰、显式授权的安全边界。
