...
1. 更新版本号，例如 `0.4.4 -> 0.4.5`。
2. 更新 `CHANGELOG.md`。
3. 运行完整本地发布闸门：`scripts/release_gate.sh`。它会依次执行 `ruff`、全量 `pytest`、`guanlan quality foundational`、`guanlan quality coverage`、`guanlan quality regression`、`guanlan quality robustness`、`guanlan eval benchmark`、`uv build`、安装 smoke 和版本核对。
4. 提交代码并推送到 `main`。
5. 打 tag 并推送，例如：
