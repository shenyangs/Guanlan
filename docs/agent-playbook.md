...
如果诊断输出 `browser_assist.recommended=true`，说明公开读取不足但目标页可能仍有样本价值。Agent 必须先问用户是否允许读取浏览器可见页面内容；允许后也只能读取目标页可见文本、标题、URL、作者和时间，不读取 Cookie、Token、钥匙串、私信、订单、后台或无关个人资料，不执行点赞、评论、关注、发帖、私信、下单或提交表单。

推荐外显说法：

> 观澜已经完成信源路线判断，当前公开读取拿到的正文不足。下一步建议使用浏览器可见页面做补证。是否允许我只读取目标页面的可见内容？我不会读取 Cookie、密码、钥匙串、私信、订单或后台信息，也不会执行任何写操作。
