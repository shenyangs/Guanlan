...
- 扩展高价值信源：Linux.do、Hacker News、CISA KEV、USGS 显著地震，以及学术发现、标准规范、开发者社区、网络安全和产品评价的高质量站点池。
- 新增 `standards` 垂直范围，并将 RFC、标准、合规与规范类问题路由到相应的官方与标准组织来源。
- 技术热榜补充 Linux.do 与 Hacker News；告警入口可汇总 CISA 已知在野利用漏洞和 USGS 显著地震。
- 增加 1.0 发布准备清单，明确稳定 API、兼容性、质量门禁与分发同步的收口条件。

### Changed

- Linux.do 公共入口在 HTTP/1.1 受限时使用有界 HTTP/2 RSS 兼容读取，保留 JSON 优先与可审计的降级边界。
- 更新信源清单、Agent 指南与路由回归样例，覆盖新增标准与学术发现路径。

### Verification

- 覆盖新增原生热榜/告警端点的真实公共读取、路由与信源注册回归；完整发布门禁、构建和安装 smoke 见本次发布记录。

## v0.10.9 - 2026-08-24

### Fixed

- 清理遥测服务滚动重启时健康探针预期内的首次连接拒绝输出；探针仍会最多等待 12 秒，但成功部署只输出可验证的备份路径。

### Verification

- 重新执行完整质量门禁、隔离安装、无噪声部署探针和线上健康验证。

## v0.10.8 - 2026-08-24

### Fixed

- 修正留存历史回放完成后仍可能在 API 元数据中显示 99% 的边界不一致；`complete=true` 现在始终对应 `processed=total` 和 `percent=100`。
- 遥测部署健康探针改为在服务重启后最多等待 12 秒，避免采集器完成安全初始化前的一次短暂拒绝连接被错误报告为部署失败。

### Verification

- 新增完成态 100% 一致性回归，并重新执行遥测专项、完整质量门禁、隔离安装和线上健康/面板验证。

## v0.10.7 - 2026-08-24

### Fixed

- 遥测留存分析改为基于持久化匿名日活读模型计算，移除每次看板刷新对全量生命周期日志重复执行十组日期 CTE 的路径；历史数据按可中断、可续跑的小批次回放，新写入同步更新读模型。
- 留存回放期间不再显示无含义的省略号，改为显示“建立中”和已处理进度；完整回放前不展示可能失真的留存率。
- 反馈面板增加最新接收时间和采集边界说明，明确反馈是显式提交或 opt-in 自动反馈，避免把没有近期反馈误判为事件遥测停摆。
- 采集器部署探针改为读取服务实际的 `GUANLAN_PORT`，兼容 Nginx 反代到非 8080 的本地监听端口。

### Verification

- 新增历史分批回放、增量写入同步、留存 cohort 一致性与占位状态语义回归；发布前继续执行完整测试、构建安装、生产备份回放与线上受控验证。

## v0.10.6 - 2026-08-24

### Fixed

- 修复大库采集器重启时无条件重复执行全量 agent-id 回填与生命周期去重的问题；已有唯一索引现在作为完成迁移标记，正常重启不再因历史清理长期不监听端口。

### Verification

- 新增“已有唯一生命周期索引时不得重跑启动迁移”回归，并重新执行遥测专项、完整质量门禁、隔离构建安装和线上健康验证。

## v0.10.5 - 2026-08-24

### Fixed

- 修复遥测面板把离线队列补传误记为“最近 24 小时”使用的问题：健康、时长、错误和未闭环指标改按客户端事件发生时间计算，接收时间只用于投递延迟诊断。
- 修复慢统计首次渲染阻塞和超时后重复启动刷新 worker 的刷新风暴；重型统计改为单飞后台任务，页面优先返回最近完整快照或明确的刷新状态。
- 修复核心卡片数值为 `0` 时被渲染为空白的问题；新增事件时间索引，并将 WAL 初始化移出每次连接，降低采集写入被看板读操作拖慢的风险。
- 遥测客户端与采集器默认确认但丢弃超过七天的生命周期积压事件，避免历史补传继续污染实时看板或无限放大数据库负担。

### Verification

- 新增延迟补传、陈旧队列、零值卡片和卡住刷新单飞回归；完整测试门禁通过后，线上按“数据库备份→校时→可回滚部署→健康验证”切换。
...
- 分离 Agent 推荐预算与工具参数合法范围：`research.read_top` 默认仍建议 `0-2`，MCP/HTTP 接受范围放宽为 `0-5`，避免合法深读请求在进入运行时前被 Schema 拒绝。
- 修复汽车销量、现金储备、产品规划等产业查询被天气或娱乐关键词误命中的问题，并增加近误判回归样例。
- Compare 自动拆分实体与共同关注维度，避免 Agent 把整段研究问题重复塞入每个对象后触发冗余搜索。

### Changed

- Agent Planner 增加工具调用总量、搜索轮次、并发、重试、深读和 Compare 对象数的机器可读预算，并补充可直接复用的执行规范文档。

## v0.8.3 - 2026-07-26

### Fixed

- 修复状态面板健康指标跨缓存窗口混算导致的异常率短暂失真，并区分真实执行错误、外层中止和未闭环调用。
- 补齐发布站点的 Cloud Assistant 回退路径，避免本机 VPN/TUN 影响 SSH 时把服务端成功部署误判为失败。
- 质量报告补充路由 scope 规则索引，使高风险路由的正例、近误判与排除条件可审计。

## v0.8.2 - 2026-07-26

### Fixed

- 修复分发状态检查在 Python 本地证书链异常时把 PyPI 与 Homebrew 校验误判为不可达的问题；保留 TLS 校验并回退到系统 `curl` 复核。
- 同步安装路径检查，避免旧全局可执行文件掩盖当前发布版本。

## v0.8.1 - 2026-07-24

### Fixed

- 修复跨入口搜索参数兼容、科技开发者热榜路由和 AI 模型显式月份查询，减少无关候选与可避免的调用中断。
- 搜索诊断补充总耗时和后端尝试耗时，便于区分上游阻断、低相关回退与外层超时。
- 修复健康面板将尚未结算调用过早计入未闭环率、空会话被错误合并的问题；状态命令不再展示内部采集配置。
- README、完整指南、相关文档和官网版本面同步到 `0.8.1`。

## v0.8.0 - 2026-07-24

### Changed

- 建立确定性可靠性基线与 CI 发布门禁：回归、鲁棒性、benchmark 和评测套件只允许保持或提升，公网波动仍独立记录，不把网络偶发性伪装成质量结论。
- 收口核心 CLI、MCP 与 HTTP 的请求归一化和错误诊断；搜索、读取、研究、路由、站内地图、日报、Agent 决策卡共享同一组参数契约，减少多入口漂移。
- 抽出公共网络执行层，并将页面读取、RSS/热榜、站点地图与公开频道接入稳定的超时、访问受限、解析与上游诊断边界；代表页证据包不再回显原始异常文本。
- 为信源 ownership、路由高风险类目覆盖和 Web legacy 兼容层增加可审计清单与结构护栏；公开质量报告同步展示可靠性基线与剩余兼容入口。
- README、完整指南、遥测文档、安全策略、官网与公开质量报告版本面同步到 `0.8.0`。

## v0.7.9 - 2026-07-03

### Changed

- 新增 Web 后端能力与抽取契约：`read` JSON 会标注正文读取、搜索上下文兜底、长文截断和可引用边界，帮助 Agent 内部补证而不向用户误报工具错误。
- README、完整指南、遥测文档和官网版本面同步到 `0.7.9`。

## v0.7.8 - 2026-07-03

### Changed

- 泛化修复 WPS 普通查询、负面短词和未知产品词的搜索逻辑：`wps_office` 不再默认把普通查询硬收窄到官网/产品入口。
- WPS 查询按 `navigation / reputation / open` 三类处理：只有明确要官网、下载、价格页时才锚定官方入口；口碑、收费、投诉、争议和普通产品解释优先开放网页证据。
- 短负面查询会保留原词并补充会员、收费、吐槽、投诉、用户评价等通用上下文，避免 `WPS 套娃` 这类查询因信息量过低返回空结果。
- 官网入口和低价值 WPS SEO 页不再计作开放证据足够候选，并从非导航查询前排移到外部报道/讨论之后。
- 补充 WPS 路由回归测试，覆盖 `WPS 套娃`、`WPS comate`、官方候选不应提前满足开放搜索等场景。
- README、完整指南、遥测文档、安全策略和官网版本面同步到 `0.7.8`。
...
- 增强世界杯搜索专项路由：`世界杯`、`美加墨`、`淘汰赛`、`比赛城市`、`球场` 等查询优先进入 `sports`，避免被本地生活/地点意图抢走。
- 为 2026 世界杯赛程/赛果补充 FIFA 官方、ESPN、FOX Sports、Olympics.com 代表入口，并保持 UEFA 欧冠入口不污染世界杯任务。
- 补充世界杯赛程、城市、球场场景的路由、直达源、搜索兜底和 JSONL 回归测试。
- README、完整指南、遥测文档和官网版本面同步到 `0.7.6`。

## v0.7.5 - 2026-06-30

### Changed

- 修复 WPS 外部报道、争议、口碑类搜索被办公产品入口过度吸附的问题，保留媒体与开放网页证据的召回空间。
- 增强中文媒体站点定向识别，让光明网、新华网、人民网、中新网等被点名时更容易进入对应证据路径。
- 补充搜索路由与排序回归测试，防止官方入口种子压住外部报道或用户样本。
- README、完整指南、遥测文档和官网版本面同步到 `0.7.5`。

## v0.7.4 - 2026-06-30

### Changed

- 融合 AI 官方与独立媒体信源层，新增 `ai-official` / `ai-media` feed 入口，并将其接入路由、日报、watch、MCP 和工具面描述。
- 扩展 AI/WPS/开发者类任务的来源身份、风险标签和 Agent 使用说明，区分公司一手更新、媒体报道与垂直社区样本。
- 补充信源包、taxonomy 与回归测试，避免把泛 AI feed 当成单一搜索结果或无边界证据。
- README、完整指南、遥测文档和官网版本面同步到 `0.7.4`。

## v0.7.3 - 2026-06-30

### Changed

- 修复命令被宿主进程超时或终止时可能只留下启动事件的问题，让异常结束统计更接近真实运行状态。
- 加强尾事件投递重试，并在运维面板中拆分“见过心跳”和“无心跳”的异常结束来源，方便定位网络抖动、宿主超时或长任务中断。
- README、完整指南、遥测文档和官网版本面同步到 `0.7.3`。
...
- 扩展 Agent 自动路由对宏观金融、泛金融配置、技术/公司一手源、医学/高校/签证官方源和阅读/文娱推荐的识别。
- 修复“固收加”“财经要闻”“美债收益率”等泛金融主题被误导到股票结构化入口的问题。
- 新增 13 条真实错误包回归样例，覆盖 PBC 逆回购、Tesla AI6、Go 1.23、Gemini Deep Research、AASM、上海交大安泰导师和泰国 DTV 签证等场景。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.18`。

## v0.6.17 - 2026-06-09

### Changed

- 修复了 `daily` watch 场景测试会被外部 feed 可用性污染的问题，并让通用 `ci` 不再因 `v*` tag push 额外挂红叉。
- 浏览器补证新增社交平台证据协议，按可见页边界补充作者、互动指标、评论样本等结构化线索。
- 遥测面板将调用统计主指标调整为 `30d 调用`，更适合观察近月使用趋势。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.17`。

## v0.6.16 - 2026-06-02

### Changed

- 修复了一个影响 Agent 搜索稳定性的不起眼问题。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.16`。
...
- 修复了一个影响 Agent 搜索稳定性的不起眼问题。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.15`。

## v0.6.14 - 2026-05-31

### Changed

- 修复了一个影响 Agent 搜索路由稳定性的不起眼问题。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.14`。
...
- 修复了一个影响 Agent 搜索路由稳定性的不起眼问题。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.13`。
...
- 修复了一个影响官网展示效果的不起眼问题。
- README、完整指南、遥测文档和官网版本面同步到 `0.6.12`。
...
- 修复了一个影响 Agent 搜索路由稳定性的不起眼问题。
- README、Agent 指南、Skill、能力地图和官网版本面同步到 `0.6.11`。

## v0.6.10 - 2026-05-26

### Changed

- 修复了一个影响 Agent 研究链路稳定性的不起眼问题。
- README、Agent 指南、Skill、能力地图和官网版本面同步到 `0.6.10`。
...
- 修复近期/今日类中文搜索在部分上游受限或召回漂移时容易返回空结果的问题。
- 新增搜索回归，确保当天日期过窄时会保留可标注的背景线索，并明确不能把背景材料写成今日事实。
...
- 修复搜索弱召回场景下容易返回空结果的问题，并加强中文复合词、官方入口和后端漂移兜底。
- 官网和版本面同步到 `0.6.5`。
...
- 新增 AnySearch 汇聚入口：支持在统一命令里组合多搜索后端并输出来源边界，方便 Agent 在复杂查询中稳定扩展候选池。
- 加强搜索语义与结果路由细粒度控制，降低拆词污染和跨域漂移，补齐对应测试与 Agent 使用指引。
- 官网和版本面同步到 `0.6.3`。
...
- 新增压力报告回放与语义补强：补充 `stress_replay` 回放数据与脚本，增强短语组合词、品牌词和专名的查询改写，降低拆字式噪声。
- 搜索质量门限补强：新增词典释义漂移污染识别、高噪声样本回归与路由回归用例，减少低价值结果误入主证据池。
- 官网和版本面同步到 `0.6.2`。
...
- 补齐 Agent 持久记忆面：新增路由回归集、live-smoke 趋势和敏感配置展示边界说明，帮助下游 Agent 把自动挡质量治理沉淀成可复测规则。
- 官网和版本面同步到 `0.6.1`。
...
- 新增 `guanlan agent` / MCP `guanlan_agent` 自动挡入口，把 route/workflow 判断压成 `primary_command`、后续步骤、质量触发器和静默补证命令，降低下游 Agent 在命令面前的选择成本。
- 新增公开舆情、品牌负面、竞品监测、价格观察、评论挖掘和应用商店评论的观澜原生 recipe，把公开讨论、评论样本、公司一手资料、垂类媒体和热榜线索组织成可审计证据链。
- 新增舆情/危机/竞品/价格/评论/应用评价路由意图、查询改写、边界提醒和建议命令，帮助 Agent 在相关任务中先做信源分层，而不是退化成一次泛搜。
- 精选补强公开讨论、应用商店/评价样本和竞品观察信源包，并将其接入 `social_web`、`market_review`、`business`、`industry_analysis` 等既有 scope。
- 新增 live smoke 历史趋势和路由回归样例，用于持续观察公网源站、中文复合词、垂类误路由和近似意图漂移。

### Changed

- 收紧宽泛词误路由：品牌投诉和负面舆情不再因“投诉”自动进入法律主路由，普通舆情词也不会把任务误带入财经情绪/股票入口。
- 加固安装与发版同步检查：PyPI 版本判断改为 JSON 与 simple index 双面确认，Agent 更新命令默认刷新 uv 索引并显式使用 PyPI 安装源。
- README、Agent 指南、完整手册、Skill、能力地图和官网同步新增自动挡、舆情、竞品、评论类推荐入口。
- 官网和版本面同步到 `0.6.0`。
...
- 公众号读取吸收批量导出工具的正文容器清洗思路，增强 `guanlan read` 对公开 `mp.weixin.qq.com` 文章标题、作者、发布时间和正文的稳定抽取。
- 新增可选的 WeChat exporter 授权适配入口，用于检查用户自配公众号导出服务，并在单独授权后查询公众号、文章列表或下载单篇文章，不默认读取 Cookie、credentials 或浏览器存储。
- 官网和版本面同步到 `0.5.42`。
...
- 股票结构化层扩展到 ETF/基金净值：`guanlan stock quote/detail/search` 可识别 `024051` 这类基金/ETF 联接基金代码和名称，输出净值日期、估算时间、基金类型、基金公司、基金经理和净值口径边界。
- Agent/MCP/Skill/README 的财经入口同步覆盖股票、指数、ETF、基金、净值和联接基金，避免下游 Agent 查基金代码时误走泛搜或外部动态页。
- 官网和版本面同步到 `0.5.41`。
...
- 大幅增强股票/财经任务的 Agent 可发现性和稳健性：新增 `guanlan stock plan` / `guide` 作为股票问题入口，`route`、`workflow`、MCP、能力说明、欢迎页、Recipe、Agent 文档和 Skill 会主动提醒先走结构化股票数据，再分层补公告、宏观、新闻、研报和情绪样本。
- 股票行情增强时效诊断、下一步命令建议和 A 股备用行情源；股票搜索在上游不可用时可用本地代码/别名识别兜底，避免 Agent 把动态财经页或临时后端失败当成“无结果”。
- 股票 quote 输出补充稳定的 `status` 与 `quote_time` 字段，方便 Agent/MCP/自动化按时效字段直接取证。
- 官网和版本面同步到 `0.5.40`。
...
- 收紧 Agent 对外补证口径：超时、上游慢或 WebFetch/浏览器可见页补证只按证据边界说明，不再引导下游把 Guanlan 写成“崩了”“抽风了”。
- 官网和版本面同步到 `0.5.39`。
...
- AI/WPS/Agent/大模型相关研究自动纳入 AI 垂类精选动态源，作为近 7 天线索层；用户不需要记新的 feeds 命令，关键事实仍回读原始 URL。
- 官网版本与静态资源版本同步到 `0.5.29`。

## v0.5.28 - 2026-05-12

### Changed

- 增强浏览器辅助补证的会话契约、动态页就绪信号和采集充分性边界，帮助 Agent 更稳地处理小红书/Rednote 等可见页补证。
- 官网版本与静态资源版本同步到 `0.5.28`。
...
- 明确后续公开更新文案、提交标题和发布说明默认聚焦搜索、阅读、路由、补证、归档、稳定性和官网体验，不主动引入无关内部实现口径。
- 清理本地官网目录跟踪摩擦，保持 `website/` 作为已跟踪官网源码时可正常随版本同步。
- 官网版本与静态资源版本同步到 `0.5.15`。
...
- 固定搜索 trace 污染诊断测试的后端网络隔离，避免 CI 上 Bing generic 实时补跑导致矩阵误报。

### Changed

- 官网版本与静态资源版本同步到 `0.5.13`。
...
- 新增精选信源包 `source_packs`：把政策、科技、财经、文娱、开发者、高校等高价值中文信源分层沉淀为 `core` / `vertical` / `sample`，用于增强主路由、scope 搜索和 Agent 选源。
- 新增随包热点目录 `hotboard:*`：本地目录可直接用于平台级热点入口发现，显式详情读取保留额度与缓存元数据。
- 新增信源包清单报告，完整列出进入主路由、scope 和路由推荐逻辑的精选信源、分层、证据角色和测试保护。

### Changed

- 路由推荐改为优先使用精选信源包：高置信源进入主路由，中低置信源只做发现、补充和样本，避免热点目录反向污染事实证据链。
- 对外文档和 Agent 指南统一使用“热点目录 / hotboard”口径，让 Agent 直接把它当作观澜的热点入口能力使用。
- `README`、Agent 文档、Skill、热点目录文档和打包数据路径同步中性化命名。

### Fixed

- 热点目录输出统一归一到 `hotboard:*`，让 CLI、Agent 文档和路由推荐保持一致。
- 随包热点目录移除非必要外部 logo 字段，保持对外资产干净。
