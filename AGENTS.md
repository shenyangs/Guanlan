...
Workflow decision rule: use `guanlan agent "query" --json` (or MCP/HTTP `guanlan_agent`) when the Agent is unsure which Guanlan command to run and wants the smallest safe command chain. Treat the result as an `agent_plan_v2` decision card: execute `primary_command` first, then review the Guanlan JSON/trace/error summary with `guanlan agent "query" --phase review --observation-json result.json --json` or MCP/HTTP `phase=review`. Follow `next_decision=answer|continue|repair|ask_user|authorize_browser|stop`; if review returns repair, run its `next_commands` or `silent_repair_commands` before answering and present the result as evidence-bound补证, not as Guanlan failure. Use `guanlan workflow "query"` when the Agent specifically needs the light/heavy decision contract without a command shortlist. Keep simple lookups direct; do not run `investigate` for basic website/link/fact searches. Use `guanlan investigate "query" --limit 80 --format context` only when the user explicitly needs deep research, cross-source evidence, comparison, timeline, dossier, high-impact verification, or a reusable evidence packet.
...
- For technology/AI/developer routing, always include one RSS discovery pass. Prefer `guanlan search ... --scope tech_dev --limit 80 --trace` plus `guanlan feeds curated --category ai --limit 80`; for AI/company/platform update tasks add `guanlan feeds ai-official --limit 80` and `guanlan feeds ai-media --limit 80`. Use `guanlan research ... --preset tech --read-top 0-2 --max-search-jobs 2` only for explicit deep evidence-packet work. Feed discovery does not replace original URLs, official docs, code repositories, announcements, or product pages.
