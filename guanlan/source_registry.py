...
_HOTNEWS_NATIVE: tuple[SourceEntry, ...] = (
    ...
    SourceEntry(
        ...
        status="stable",
        evidence_role="multi_source_snapshot",
        notes="聚合 IT之家、新智元、V2EX、Linux.do、Hacker News 与全球开发者链接；单源失败不影响其它来源。",
        caveat="用于发现科技和开发者圈的公开线索，重要事实仍需回读原文。",
        aliases=("tech_dev", "developer"),
    ),
    SourceEntry(
        id="alerts",
        name="官方安全与灾害预警",
        ...
        backend="native",
        status="stable",
        evidence_role="official_alert_snapshot",
        notes="聚合 CISA 已知在野利用漏洞目录与 USGS 显著地震；单源失败不影响另一来源。",
        caveat="预警快照用于发现高影响事件；影响范围和处置建议仍应回读对应官方记录。",
        aliases=("official-alerts", "risk-alerts"),
    ),
    SourceEntry(
        id="baidu",
        name="百度热搜",
    ...
    SourceEntry(
        ...
        freshness="minutes",
        quality="first-party public RSS",
        route_when="用户问全球开发者和创业者社区当前关注的技术与产品链接。",
        command="guanlan hotnews hackernews --limit 80",
        caveat="社区排序和讨论热度只代表 HN 用户样本，不等于事实权威性。",
        risk_tags=("community_bias", "sample_bias"),
        aliases=("hn",),
    ),
    SourceEntry(
        id="linuxdo",
        ...
        status="best-effort",
        evidence_role="developer_discussion_signal",
        source_domain="linux.do",
        freshness="minutes",
        quality="public Discourse JSON with first-party RSS fallback",
        route_when="用户问中文开发者、AI 工具和开源社区最近在讨论什么。",
        command="guanlan hotnews linuxdo --limit 80",
        caveat="公开社区样本可能受登录墙、反爬和社区偏好影响，不能替代项目或厂商原文。",
        risk_tags=("community_bias", "sample_bias", "access_volatility"),
    ),
    SourceEntry(
        id="cisa-kev",
        name="CISA 已知在野利用漏洞目录",
        surface="hotnews",
        platform="cisa",
        category="cybersecurity",
        backend="native",
        status="stable",
        evidence_role="official_security_alert",
        source_domain="cisa.gov",
        freshness="daily",
        quality="official CISA JSON feed",
        route_when="用户问近期已确认被利用的 CVE、补丁优先级或勒索风险。",
        command="guanlan hotnews cisa-kev --limit 80",
        caveat="KEV 证明漏洞已知被利用，不自动证明某个用户环境受影响；需结合资产版本和厂商公告。",
        risk_tags=("fast_changing", "requires_asset_context"),
        aliases=("kev", "cisa"),
    ),
    SourceEntry(
        id="usgs-earthquakes",
        ...
        backend="native",
        status="stable",
        evidence_role="official_disaster_alert",
        source_domain="earthquake.usgs.gov",
        freshness="minutes",
        quality="official USGS GeoJSON feed",
        route_when="用户问近期全球显著地震、震级、位置和海啸标记。",
        command="guanlan hotnews usgs-earthquakes --limit 80",
        caveat="显著地震目录会随复核更新；当地风险和处置以当地权威机构最新通告为准。",
        risk_tags=("fast_changing", "location_context_required"),
        aliases=("earthquakes", "usgs"),
    ),
    SourceEntry(
        id="xinzhiyuan",
        name="新智元",
...
_FEEDS: tuple[SourceEntry, ...] = (
    ...
    SourceEntry(
        ...
        name="AI 官方更新流",
        surface="feeds",
        platform="official-rss",
        category="ai",
        backend="native",
        status="stable",
        evidence_role="official_ai_update_signal",
        content_direction="OpenAI、Google DeepMind、Google AI、Hugging Face、GitHub AI/Microsoft/NVIDIA 等官方 RSS/Atom 更新。",
        freshness="dated",
        quality="first-party public RSS/Atom bundle with per-source boundaries",
        route_when="用户问 AI 模型、产品、平台、开发者工具的正式发布、更新或官方口径时，用作一手线索层。",
        command="guanlan feeds ai-official --limit 80",
        confidence="medium-high",
        caveat="这是官方更新入口集合；不同公司口径只代表自身发布，不能替代外部报道、社区样本或监管/安全核验。",
        risk_tags=("company_primary", "source_requires_original_verification"),
        aliases=("official-ai", "ai-official-rss", "ai-news-official"),
    ),
    SourceEntry(
        id="ai-media",
        name="AI 媒体观察流",
        ...
        backend="native",
        status="best-effort",
        evidence_role="ai_media_report_signal",
        content_direction="The Decoder、TechCrunch AI、VentureBeat AI、Wired AI、MarkTechPost 等公开 RSS 的 AI 媒体/研究线索。",
        freshness="dated",
        quality="curated public AI media RSS with strict filters on broad feeds",
        route_when="用户问 AI 行业观察、媒体报道、产品/模型外部报道或需要补官方之外的 B 层来源时。",
        command="guanlan feeds ai-media --limit 80",
        confidence="medium",
        caveat="媒体流适合补外部报道和行业视角；标题和摘要不能直接当结论，关键事实需回读原文。",
        risk_tags=("media_framing", "editorial_selection", "source_requires_original_verification"),
        aliases=("curated-ai-media", "ai-news-media", "ai-media-rss"),
    ),
    SourceEntry(
        id="curated-sources",
        name="精品源目录",
