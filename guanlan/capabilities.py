...
CAPABILITIES: tuple[Capability, ...] = (
    Capability(
        ...
        cli=["guanlan welcome", "guanlan capabilities", "guanlan capabilities --json", "guanlan agent \"用户需求\" --json"],
    ...
    Capability(
        id="agent_auto",
        name="Agent 自动挡",
        description="把用户需求压缩成一条主命令和少量后续命令，让 Agent 不必在完整功能面里猜。",
        when_to_use="Agent 不确定该直接 search、升级 research、补 hotnews/feeds，或只想要一个低选择成本起手式时。",
        cli=[
            "guanlan agent \"用户需求\" --json",
            "guanlan agent \"WPS AI 灵犀 最近热点\" --mode fresh --json",
            "guanlan agent \"某公司 风险 舆情 档案\" --mode deep --json",
        ],
        mcp="guanlan_agent",
        boundary="本地规划层，不联网、不替代 search/read/research；执行 primary_command 后再按质量信号决定是否补跑 agent_next_steps。",
        examples=["我不知道该用哪个 Guanlan 命令，给我一个起手式。", "这个任务让 Agent 自动选路线。"],
    ),
    Capability(
        id="route",
        name="需求与信源路由",
    ...
    Capability(
        id="recipes",
        name="研究 Recipe",
        description="把高校导师、财经风险、产品口碑、公开舆情、品牌负面、竞品监测、评论挖掘、文娱热度、安全公告、技术趋势、WPS/AI Office 选题等常见任务固化为可复用工作流模板。",
    ...
    Capability(
        id="daily",
        name="日报简报",
        description="把 route、search/watch、feeds 和 hotnews 串成多源采编日报，按主线聚合事实、风险、社区样本、动作建议和候补线索。",
        when_to_use="用户要每天看、要早报/日报、要品牌/市场/舆情简报，或 Agent 需要把公开中文互联网线索压成一份带来源边界的 briefing 时。",
        cli=[
            "guanlan daily",
            "guanlan daily \"WPS AI PPT Agent 办公选题\" --time-window 3d --read-top 3",
            "guanlan daily \"某品牌 最近舆情\" --edition reputation --format im",
            "guanlan daily --watch-id openai-api-release-notes --record-history --compare-days 7",
            "guanlan daily \"品牌设计趋势 2026\" --format html --output ./guanlan-daily.html",
        ],
        mcp="guanlan_daily",
        boundary="日报主正文不由 SEO/下载/镜像等 D 层弱线索撑起；官方口径不等于全网情况，社区层只能作为样本，editorial_health=block 时只能当线索池。",
        examples=["给我一份今天的中文互联网日报。", "每天帮我做一份 AI 行业简报。", "把这个 watch 直接生成为日报。"],
    ),
    Capability(
        id="market_intelligence",
        name="公开舆情与竞品观察",
        description="把品牌/产品的公开讨论、评论样本、应用商店评分、竞品官网/价格页、媒体报道和热榜线索组织成可审计证据链。",
        when_to_use="用户问舆情、风评、声量、危机、差评、应用评论、竞品情报、定价变化、同类产品或市场格局时。",
        cli=[
            "guanlan route \"某品牌 负面舆情 投诉 最近\" --json",
            "guanlan recipe run public-opinion-pulse \"某产品 最近风评\"",
            ...
            "guanlan recipe run app-review-pulse \"某 App 应用商店评论 评分变化\"",
            "guanlan pulse \"某产品 最近风评\" --limit 80 --format context",
        ],
        mcp="guanlan_route / guanlan_recipe / guanlan_pulse / guanlan_research",
        boundary="只读公开信息；社交、评论和应用商店样本不代表总体比例。平台登录态、私域页面或浏览器补证必须先单独授权，并保留可见页/样本边界。",
        examples=["这个品牌最近是不是翻车了？", "竞品最近价格和功能有什么变化？", "App Store 差评主要在骂什么？"],
    ),
    Capability(
        id="search",
        name="互联网搜索",
    ...
    Capability(
        id="read",
        name="网页阅读",
        description="把 URL 读取成 Agent 可用 Markdown；公众号文章会先走公开文章专项抽取，再按 Jina Reader、直连 HTML、搜索兜底逐级降级。",
        when_to_use="用户给了链接，或 research 需要读取代表性原文。",
        cli=[
            "guanlan read \"https://example.com/article\" --max-chars 12000",
            "guanlan read \"https://mp.weixin.qq.com/s/...\" --trace --max-chars 12000",
            "guanlan read \"URL\" --backend direct",
        ],
        mcp="guanlan_read",
        boundary="只读公开页面；公众号专项抽取不读取 Cookie/Token/credentials/浏览器存储。登录墙、反爬、403 时降级，不自动读取 Cookie。",
        examples=["读一下这篇文章。", "Jina 读出来噪音太多，试试直连。"],
    ),
    Capability(
        id="wechat_exporter",
        name="公众号授权导出适配",
        description="连接用户自配的 WeChat article exporter 服务，用 X-Auth-Key 查询公众号、历史文章列表或下载单篇文章。",
        when_to_use="用户已经配置公众号导出服务，并明确要查某公众号历史文章、批量文章链接或把公众号资料整理进证据包时。",
        cli=[
            ...
            "guanlan wechat-exporter download \"https://mp.weixin.qq.com/s/...\" --format markdown",
        ],
        mcp=None,
        status="opt-in",
        boundary="可选授权适配器；只读取用户配置的 GUANLAN_WECHAT_EXPORTER_BASE_URL 和 GUANLAN_WECHAT_EXPORTER_AUTH_KEY，不读取浏览器 Cookie、credentials、IndexedDB 或公众号后台页面。",
        examples=["帮我列出这个公众号最近文章。", "用我自配的公众号导出服务下载这篇文章。"],
    ),
    Capability(
        id="research",
        name="研究证据包",
        description="受控重型证据包工具：把搜索、路由、代表证据、可选原文摘录、来源分布和证据审计整理成 Agent-ready evidence packet。",
        when_to_use="用户明确需要深查、可复用证据包，或 search + read 后仍缺信源角色覆盖；普通任务先用 search/read/feeds/hotnews。",
        cli=[
            "guanlan research \"关键词\" --profile china --read-top 0 --max-search-jobs 2",
            "guanlan research \"OpenAI API pricing release notes\" --preset company --profile english",
            "guanlan research \"政策问题\" --preset policy --read-top 0 --max-search-jobs 2",
            "guanlan research \"Python Agent 框架 对比\" --preset tech --read-top 0 --max-search-jobs 2",
            "guanlan research \"WPS AI PPT Agent 办公选题 最近热点\" --preset wps_office --read-top 0 --max-search-jobs 2 --advisor",
        ...
        boundary="MCP/Agent 场景默认保持 read_top 0-2，并用单独 read 补代表 URL；科技/AI/WPS 路线仍需 feeds/RSS 发现，不替代官方文档、代码仓库或原文核验。",
        examples=["帮我查清楚这个行业趋势。", "整理一下这个产品的公开口碑。"],
    ),
    Capability(
        id="wps_office",
        name="WPS/AI Office 路由",
        description="围绕金山办公、WPS、WPS AI、WPS 365，并外扩办公 AI、PPT、Agent、文档协作、SaaS、信创、安全、竞品和热点线索。",
        when_to_use="金山办公品牌市场、办公软件/AI Office 赛道、PPT 生成、办公 Agent、企业协作、信创办公、安全合规和相关选题雷达任务。",
        cli=[
            "guanlan route \"WPS AI PPT Agent 办公选题 最近热点\" --json",
            "guanlan search \"办公 AI PPT 文档协作 SaaS 信创\" --scope wps_office --limit 80 --trace",
            "guanlan feeds curated --category ai --limit 80",
            "guanlan research \"WPS AI PPT Agent 办公选题 最近热点\" --preset wps_office --limit 80 --read-top 0 --max-search-jobs 2 --advisor",
            "guanlan recipe run wps-office-radar \"WPS AI 选题线索\"",
            "guanlan hotnews today --limit 80 --trends",
        ],
        mcp="guanlan_route / guanlan_search / guanlan_research / guanlan_recipe",
        boundary="这不是品牌白名单搜索；官方、竞品、产业媒体、开发者/用户样本、RSS 和热榜要分层看，品牌稿和单条社媒不能作主证据。",
        examples=["帮金山办公市场部找办公 AI 选题。", "WPS 365 和 AI Office 最近有哪些行业线索？"],
    ),
    Capability(
        id="entertainment",
        name="文娱路由",
    ...
    Capability(
        id="finance",
        name="股票与财经路由",
        description="股票/指数/ETF/基金问题先走结构化 stock 数据，再把财经查询拆成行情/净值、公告披露、公司一手、财经新闻、宏观数据、研报观点和投资者情绪。",
        when_to_use="用户问股票、股价、ETF、基金净值、财报、公告、监管、宏观金融、研报、雪球/股吧情绪、公司风险或资本市场动态时。",
        cli=[
            ...
            "guanlan research \"宁德时代 股价 财报 公告 最近风险\" --preset finance --limit 80 --read-top 0 --max-search-jobs 2 --advisor",
        ],
        mcp="guanlan_stock / guanlan_route / guanlan_search / guanlan_research / guanlan_read",
        boundary="财经输出只整理公开证据、时效和风险边界；结构化行情/净值可能延迟，基金净值、估算和场内 ETF 行情口径不同；研报和社交情绪不能写成买入、卖出或持有建议。",
        examples=["帮我查这家公司最近财报和风险。", "这只股票今天为什么大跌？", "024051 是什么基金，最新净值多少？", "雪球上大家怎么看？"],
    ),
    Capability(
    ...
    Capability(
        id="feeds",
        name="精品内容发现",
        description="读取公开 RSS/Atom、AI 官方更新流、AI 媒体观察流、arXiv 预印本、动态热点 RSS、本地 watchlist 和精品源目录，发现高质量技术、AI、产品、商业科技与微信热门文章线索。",
        when_to_use="用户想看值得读的技术文章、AI 官方/媒体动态、公众号热文、arXiv 论文线索、实时热点 RSS、本地订阅源观察，或需要 RSS 内容池/源目录。",
        cli=[
...
def format_capabilities_markdown(capabilities: list[dict[str, Any]] | None = None) -> str:
    ...
    lines = [
        ...
        "- 查股票/股价/指数/ETF/基金净值/资金流：先用 `guanlan stock plan \"问题\"`，再用 `guanlan stock quote/detail/fundflow`",
        ...
        "- 要生成日报：`guanlan daily \"主题\"` 或 `guanlan daily --watch-id <id>`",
