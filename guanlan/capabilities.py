...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability(
        ...
        cli=[
            ...
            "guanlan search \"中文问题\" --profile china --limit 50 --trace",
            "guanlan search \"OpenAI API pricing\" --profile english --scope company_primary --limit 50",
...
def format_capabilities_markdown(capabilities: list[dict[str, Any]] | None = None) -> str:
    ...
    lines = [
        ...
        "- 网络超时只能说明本轮网络或上游源未完成，不代表没有证据；优先重试一次、使用 `--cache-ttl 3600`，或降低 `--read-top`，不要把正常 50 条候选池砍成小样本。",
        "",
        "## 能力清单",
        "",
    ]
