...
_QUALITY_INTENT_PROFILES: dict[str, dict[str, Any]] = {
    "policy": {
        ...
        "terms": (
            ...
            "住房公积金", "公积金贷款", "首套房", "最低首付比例", "个人住房贷款",
            "契税", "税收政策", "财政部", "税务总局", "非银行支付机构网络支付业务管理办法",
            "中医药振兴发展重大工程", "中医药传承创新发展", "中医药文化", "中医养生保健服务规范",
            "医疗广告管理办法", "广告法", "中医药法", "不得保证治愈", "不得从事医疗活动",
            "国家中医药管理局", "国家市场监督管理总局",
        ),
    ...
    "medical_health": {
        "name": "医疗/健康高影响信息",
        "terms": ("医疗", "疾病", "药", "药品", "治疗", "治未病", "中医药", "中医养生", "中医养生保健", "宣传疗效", "医疗广告", "诊断", "症状", "临床", "指南", "医生", "医院", "枕头高度", "颈椎健康", "睡姿", "睡眠医学", "aasm", "cervical lordosis", "neck pain", "pillow height", "孕期", "怀孕", "布洛芬", "肺结节", "肌酸", "视黄醇", "敏感肌", "猫咪", "呕吐", "fda", "cdc", "who", "medical", "clinical", "treatment"),
    ...
    "legal_judicial": {
        "name": "法律/司法高影响信息",
        "terms": ("法律", "诉讼", "判决", "合同", "律师", "侵权", "司法解释", "法院", "裁判文书", "条例", "广告法", "中医药法", "医疗广告管理办法", "中医养生保健服务规范", "不得保证治愈", "不得从事医疗活动", "房产纠纷", "婚前财产", "财产纠纷", "房产加名", "纠纷", "工伤", "竞业", "劳动争议", "版权", "著作权", "维权", "投诉", "数据安全法", "个人信息保护法", "爬虫", "反爬", "法律风险", "license", "agpl", "law", "legal", "court", "lawsuit"),
    ...
    "local_life": {
        "name": "本地生活/到店攻略",
        "terms": ("骑行路线", "骑行", "brunch", "咖啡店", "咖啡馆", "书店", "书吧", "深夜书店", "小店", "文艺小店", "安静小店", "晨间", "发呆", "凌晨", "深夜", "不排队", "看海", "夜景", "旅行攻略", "周末好去处", "横琴", "自驾", "景区", "赛里木湖", "喀拉峻", "夏塔", "库尔德宁", "琼库什台"),
    ...
    "design_trend": {
        "name": "设计趋势/视觉样本",
        "terms": ("流行色", "色彩趋势", "设计趋势", "产品配色", "配色趋势", "丁香紫", "pantone", "插画", "温柔插画", "ai生成", "ai 插画"),
        "preferred_scopes": ("social_web", "business", "industry_analysis"),
        "preferred_source_types": ("社交/内容平台", "商业/产业媒体", "评价/消费样本"),
        "caution_source_types": ("通用网页",),
        "guidance": "设计趋势和视觉样本应区分权威色彩/设计社区、品牌案例和平台种草；不要把单平台样本当总体趋势。",
    },
    "industry": {
        "name": "产业/公司研究",
        "terms": (
    ...
    "finance": {
        "name": "财经/资本市场",
        "terms": ("财经", "财经要闻", "股票", "股价", "财报", "融资", "上市", "投资", "基金", "债券", "宏观", "资本市场", "a股", "港股", "固收加", "固收+", "基金配置", "配置价值", "存款利率", "理财收益率", "英伟达", "nvidia", "大跌", "etf", "质押", "银行倒闭", "fedwatch", "降息", "净值", "持仓", "跟踪指数", "基金概况", "成立日期", "利空", "下跌原因", "逆回购", "买断式逆回购", "mlf", "流动性", "资金面", "债市", "美债收益率", "treasury yield", "us treasury yield", "ftse", "gilts", "bond yields", "imf", "bank of england", "贷款市场报价利率", "lpr", "住房贷款利率", "支付体系运行总体情况", "移动支付", "业务金额", "业务笔数", "国家统计局", "70个大中城市", "商品住宅销售价格", "居民收入和消费支出"),
    ...
    "tech": {
        "name": "技术/开发者",
        "terms": (
            "技术", "开源", "框架", "模型", "api", "sdk", "github", "开发者", "部署", "bug", "benchmark", "mcp", "教程",
            "自动加字幕", "剪映", "hermes agent", "内存占用", "cpu 性能测试", "vscode", "vs code",
            "visual studio code", "调试新特性", "node.js", "nodejs", "node js", "lts", "安装包", "下载地址",
            "wordpress", "woocommerce", "spectra", "seo", "seedance", "豆包", "doubao", "视频生成模型",
            "ai写作", "ai写小说", "ai绘画", "ai画画", "ai工具", "ai读书笔记工具", "读书笔记工具",
            "多agent", "多 agent", "multi-agent", "multi agent", "ai内容生成", "ai 内容生成",
            "系统架构", "实践案例", "多轮对话", "确认节点", "交互设计", "用户引导",
            "winui3", "winui 3", "windows app sdk", "sam 3", "segment anything with concepts", "ultralytics",
            "notificationlistenerservice", "notification listener service", "通知监听服务", "android 通知监听",
            "tasker", "icost", "callback url", "x-callback-url", "url scheme", *_ROBOTICS_AI_TERMS,
            "go 1.23", "golang", "range over func", "泛型", "芯片", "算力", "晶圆",
            "music generation", "音乐生成", "ai音乐", "ai 音乐", "gemini deep research", "deep research",
            "google gemini", "minimax", "cdp bridge", "coinbase", "worldarena", "leaderboard",
            "world models", "wechatferry", "openclaw", "hermesclaw",
        ),
