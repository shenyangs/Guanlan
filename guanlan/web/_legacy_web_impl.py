...
def search_web(
    ...
    quality_rejected_batches: list[tuple[str, list[SearchResult], dict[str, Any]]] = []
    ...
    for name in order:
        ...
        try:
            ...
            if batch and not batch_quality["usable"] and (backend == "auto" or name == "bing"):
                quality_rejected_batches.append((name, list(batch), dict(batch_quality)))
    ...
    if backend == "auto" and not results:
        _append_quality_salvage_results(
            results,
            diagnostics=backend_diagnostics,
            rejected_batches=quality_rejected_batches,
            original_query=original_query,
            quality=quality,
            effective_scope=effective_scope,
            route_plan=route_plan.to_dict(),
...
def _append_quality_salvage_results(
    results: list[SearchResult],
    *,
    diagnostics: list[dict[str, Any]],
    rejected_batches: list[tuple[str, list[SearchResult], dict[str, Any]]],
    original_query: str,
    quality: dict[str, Any],
    effective_scope: str | None,
    route_plan: dict[str, Any],
    ...
    """Keep bounded, clearly marked entrypoints when every normal backend is weak."""
    if results or not rejected_batches:
        return
    scored: list[tuple[float, SearchResult, str, str]] = []
    for backend_name, batch, gate in rejected_batches:
        for item in batch:
            score, reason = _quality_salvage_score(
                item,
                original_query=original_query,
                quality=quality,
                effective_scope=effective_scope,
            ...
            if score < 2.0:
                continue
            scored.append((score, item, backend_name, reason))
    if not scored:
        return
    scored.sort(key=lambda row: (-row[0], row[1].rank))
    existing: set[str] = set()
    added: list[SearchResult] = []
    for score, item, backend_name, reason in scored:
        ...
        item.trace.update(
            {
                "quality_salvage": True,
                "quality_salvage_reason": reason,
                "quality_salvage_score": round(score, 3),
                "quality_salvage_backend": backend_name,
                "agent_note": (
                    "这是弱召回场景保留下来的入口线索，不是完整答案；"
                    "需要继续 read/research 或按 external_fetch_strategy 补证。"
                ),
            }
        )
        item.evidence_role = item.evidence_role or "open_web_context"
        added.append(item)
        if len(added) >= min(max(limit, 1), 5):
            break
    if not added:
        return
    results.extend(added)
    diagnostics.append(
        {
            "backend": "quality_salvage",
            "status": "ok",
            "result_count": len(added),
            "error": "",
            "note": (
                "所有常规搜索后端均未形成稳态候选；已保留少量具备实体/权威入口信号的弱线索，"
                "避免把网络或后端漂移误报成搜索结果为空。"
            ),
            "policy": "bounded_authoritative_entrypoint_salvage",
            "sources": [
                {
                    "title": item.title,
                    "domain": item.domain or _domain(item.url),
                    "url": item.url,
                    "reason": item.trace.get("quality_salvage_reason", ""),
                }
                for item in added
            ],
        }
...
def _quality_salvage_score(
    ...
    *,
    ...
    domain = item.domain or _domain(item.url)
    if not domain or _quality_salvage_low_value(domain, item):
        return 0.0, "low_value_or_polluted"
    text = _collapse_ws(f"{item.title} {item.snippet} {item.url}")
    compact = re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", text.lower())
    anchor_terms = _quality_salvage_anchor_terms(original_query, quality=quality, route_plan=route_plan)
    matched_terms = [
        term
        for term in anchor_terms
        if term.lower() in text.lower()
        or re.sub(r"[^0-9a-z\u4e00-\u9fff]+", "", term.lower()) in compact
    ]
    try:
        from guanlan.search_sources import classify_domain

        source_meta = classify_domain(domain, preferred_scope=effective_scope)
    except Exception:
        source_meta = {"matched_scope": "", "source_type": "", "trust_level": 1}
    source_card = source_card_for_domain(domain).to_dict()
    matched_scope = str(source_meta.get("matched_scope") or source_card.get("scope_id") or "")
    source_type = str(source_meta.get("source_type") or source_card.get("source_type") or "")
    authority_role = str(source_card.get("authority_role") or "")
    trust_level = int(source_meta.get("trust_level") or 1)
    ...
    if not quality.get("requested_scope"):
        preferred_scopes.update(str(scope) for scope in route_plan.get("preferred_scopes") or [] if str(scope))
    preferred_types = {str(kind) for kind in quality.get("preferred_source_types") or [] if str(kind)}
    route_intents = {str(intent) for intent in route_plan.get("primary_intents") or [] if str(intent)}
    route_intents.update(str(intent) for intent in route_plan.get("secondary_intents") or [] if str(intent))
    score = 0.0
    reasons: list[str] = []
    if matched_terms:
        score += min(len(matched_terms), 3) * 0.8
        reasons.append("anchor_terms:" + ",".join(matched_terms[:3]))
    if matched_scope and (matched_scope in preferred_scopes or matched_scope == effective_scope):
        score += 1.4
        reasons.append(f"scope:{matched_scope}")
    if source_type and source_type in preferred_types:
        score += 1.1
        reasons.append(f"source_type:{source_type}")
    if domain.endswith(".gov.cn") or (
        matched_scope in {"gov", "local_official", "party_central", "global_official"}
        and (matched_scope in preferred_scopes or matched_scope == effective_scope)
    ):
        score += 1.2
        reasons.append("official_domain")
    if authority_role in {"company_primary", "product_primary", "government", "official_community", "developer_source"}:
        score += 0.9
        reasons.append(f"authority:{authority_role}")
    if authority_score >= 0.7 or trust_level >= 4:
        score += 0.6
        reasons.append("high_trust")
    if _quality_salvage_company_domain(domain, original_query):
        score += 1.1
        reasons.append("company_or_product_domain")
    pollution = gate.get("pollution") or {}
    if pollution.get("severity") == "high":
        score -= 0.9
        reasons.append("pollution_high")
    if not matched_terms and score < 2.6:
        return 0.0, "no_anchor_match"
    if _contains_cjk(original_query) and not matched_terms and not (domain.endswith(".gov.cn") and effective_scope in {"gov", "local_official"}):
        return 0.0, "cjk_anchor_missing"
    return score, ";".join(reasons) or "weak_entrypoint"


def _quality_salvage_anchor_terms(
    query: str,
    *,
    quality: dict[str, Any],
    route_plan: dict[str, Any],
) -> list[str]:
    terms = list(_query_relevance_terms(query))
    ...
    terms.extend(phrase for phrase in extra_phrases if phrase.lower() in query_lower)
    for role in route_plan.get("evidence_roles") or []:
        if str(role):
            terms.append(str(role))
    for term in quality.get("matched_terms") or []:
        if str(term):
            terms.append(str(term))
    return _unique_keep_order([term for term in terms if len(term) >= 2])[:24]


def _quality_salvage_low_value(domain: str, item: SearchResult) -> bool:
    ...
    if normalized in low_value_domains or any(normalized.endswith("." + domain) for domain in low_value_domains):
        return True
    text = _collapse_ws(f"{item.title} {item.snippet}").lower()
    if normalized == "baike.baidu.com" and not any(term in text for term in ("政策", "公告", "公司", "产品", "纠纷")):
        return True
    if any(term in text for term in ("客服电话", "24小时", "人工服务", "app下载", "破解版")):
        return True
    return False


def _quality_salvage_company_domain(domain: str, query: str) -> bool:
    normalized = domain.lower().removeprefix("www.")
    text = query.lower()
    known_domains = {
        "bytedance.com": ("字节跳动", "bytedance"),
        "doubao.com": ("豆包", "doubao"),
        "volcengine.com": ("火山引擎", "volcengine"),
        "openai.com": ("openai", "chatgpt"),
        "anthropic.com": ("anthropic", "claude"),
    }
    for root, aliases in known_domains.items():
        if normalized == root or normalized.endswith("." + root):
            return any(alias in text for alias in aliases)
    return False


def _duckduckgo_recovery_blocked_by_health(diagnostics: list[dict[str, Any]]) -> bool:
    for item in diagnostics:
        backend = str(item.get("backend") or "")
        status = str(item.get("status") or "")
        if backend == "duckduckgo" and status in _NETWORK_PROBLEM_STATUSES:
            return True
    return False


def _append_duckduckgo_recovery_skip(diagnostics: list[dict[str, Any]], backend: str, note: str) -> None:
    diagnostics.append(
        {
            "backend": backend,
            "status": "skipped",
            ...
            "error": "",
            "note": note,
            "policy": "skip_repeated_duckduckgo_timeout",
        }
    )
    ...
    *,
    ...
    if _duckduckgo_recovery_blocked_by_health(diagnostics):
        _append_duckduckgo_recovery_skip(
            diagnostics,
            "duckduckgo:route_target_site",
            "DuckDuckGo 主路径刚发生网络超时，跳过同后端站点补搜，避免外层 Agent/MCP 调用继续被慢后端拖到 abort。",
        )
        return
    ...
    *,
    ...
    if _duckduckgo_recovery_blocked_by_health(diagnostics):
        _append_duckduckgo_recovery_skip(
            diagnostics,
            "duckduckgo:recovery",
            "DuckDuckGo 主路径刚发生网络超时，跳过同后端多 query 恢复，避免外层 Agent/MCP 调用继续被慢后端拖到 abort。",
        )
        return
...
def _run_multi_entity_fanout_pass(
    ...
    *,
    ...
    if _duckduckgo_recovery_blocked_by_health(diagnostics):
        _append_duckduckgo_recovery_skip(
            diagnostics,
            "duckduckgo:entity_fanout",
            "DuckDuckGo 主路径刚发生网络超时，跳过多实体 fanout 补搜，避免同一慢后端重复等待。",
        )
        return
    entities = [
        entity
...
def _network_modes_for_backend(backend: str, requested: str, profile: str | None = None) -> list[str]:
    ...
    if not proxy_url:
        ...
        if requested == "auto" and "current" in modes and "direct" in modes:
            modes = ["current"]
...
def _search_duckduckgo(
    ...
    with _open_url_with_network(req, timeout=_DUCKDUCKGO_SEARCH_TIMEOUT, network_mode=network_mode) as resp:
...
# Search quality/query strategy implementation moved to guanlan.web._search_quality_impl.
...
assess_read_quality = _read_impl.assess_read_quality
build_read_quality_report = _read_impl.build_read_quality_report
format_read_quality_report = _read_impl.format_read_quality_report
read_batch = _read_impl.read_batch
read_url = _read_impl.read_url
read_url_with_trace = _read_impl.read_url_with_trace
_batch_block_reason = _read_impl._batch_block_reason
_contains_cjk = _read_impl._contains_cjk
_filter_read_fallback_results = _read_impl._filter_read_fallback_results
_format_read_watch = _read_impl._format_read_watch
_looks_like_dynamic_finance_shell = _read_impl._looks_like_dynamic_finance_shell
_read_search_context = _read_impl._read_search_context
_snapshot_path = _read_impl._snapshot_path

_search_quality_impl = importlib.import_module("guanlan.web._search_quality_impl")
detect_search_quality_profile = _search_quality_impl.detect_search_quality_profile
_quality_with_route_plan = _search_quality_impl._quality_with_route_plan
search_quality_summary = _search_quality_impl.search_quality_summary
_quality_agent_decision = _search_quality_impl._quality_agent_decision
_browser_assist_suggestion_from_results = _search_quality_impl._browser_assist_suggestion_from_results
_quality_status = _search_quality_impl._quality_status
_quality_has_strong_primary_evidence = _search_quality_impl._quality_has_strong_primary_evidence
_quality_why_cautious = _search_quality_impl._quality_why_cautious
_quality_user_facing_status = _search_quality_impl._quality_user_facing_status
_quality_followup_actions = _search_quality_impl._quality_followup_actions
_quality_execution_policy = _search_quality_impl._quality_execution_policy
_quality_workflow_plan = _search_quality_impl._quality_workflow_plan
_quality_gap_interpretation = _search_quality_impl._quality_gap_interpretation
_quality_gap_reporting_contract = _search_quality_impl._quality_gap_reporting_contract
_quality_gap_next_steps = _search_quality_impl._quality_gap_next_steps
_analyze_search_query_shape = _search_quality_impl._analyze_search_query_shape
_compress_overlong_query = _search_quality_impl._compress_overlong_query
_expand_search_query = _search_quality_impl._expand_search_query
_query_shape_entities = _search_quality_impl._query_shape_entities
_looks_like_meaningless_query = _search_quality_impl._looks_like_meaningless_query
_quality_followup_preset = _search_quality_impl._quality_followup_preset
_source_gap_suggestion = _search_quality_impl._source_gap_suggestion
_role_gap_suggestion = _search_quality_impl._role_gap_suggestion
_quality_term_matches = _search_quality_impl._quality_term_matches
detect_recency_intent = _search_quality_impl.detect_recency_intent
_explicit_year_recency = _search_quality_impl._explicit_year_recency
build_query_strategy = _search_quality_impl.build_query_strategy
_query_strategy_time_window = _search_quality_impl._query_strategy_time_window
_apply_recency_query = _search_quality_impl._apply_recency_query
_query_already_has_absolute_date = _search_quality_impl._query_already_has_absolute_date
_recency_term_matches = _search_quality_impl._recency_term_matches
_recency_today = _search_quality_impl._recency_today
